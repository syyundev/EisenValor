...
std::wstring Utf8ToWide(const std::string_view text)
{
	...
	MultiByteToWideChar(
		CP_UTF8, MB_ERR_INVALID_CHARS, text.data(), static_cast<int>(text.size()), result.data(), requiredSize
	);
	return result;
}
...
LRESULT CALLBACK LoginDialogProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		const std::wstring initialID = result ? Utf8ToWide(result->id) : std::wstring{};
		CreateWindowExW(0, L"STATIC", L"ID:", WS_CHILD | WS_VISIBLE, 20, 36, 45, 20, hwnd, nullptr, nullptr, nullptr);
		CreateWindowExW(
			0, L"EDIT", initialID.c_str(), WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, 70, 32, 200, 24, hwnd,
			reinterpret_cast<HMENU>(static_cast<INT_PTR>(kIdEditID)), nullptr, nullptr
		);
		CreateWindowExW(0, L"STATIC", L"PW:", WS_CHILD | WS_VISIBLE, 20, 76, 45, 20, hwnd, nullptr, nullptr, nullptr);
		CreateWindowExW(
			0, L"EDIT", L"", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL | ES_PASSWORD, 70, 72, 200, 24, hwnd,
			reinterpret_cast<HMENU>(static_cast<INT_PTR>(kPwEditID)), nullptr, nullptr
		);
		CreateWindowExW(
			0, L"BUTTON", L"Register", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 16, 132, 75, 28, hwnd,
			reinterpret_cast<HMENU>(static_cast<INT_PTR>(kRegisterButtonID)), nullptr, nullptr
		);
		CreateWindowExW(
			0, L"BUTTON", L"Login", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON, 165, 132, 60, 28, hwnd,
			reinterpret_cast<HMENU>(static_cast<INT_PTR>(kLoginButtonID)), nullptr, nullptr
		);
		CreateWindowExW(
			0, L"BUTTON", L"Cancel", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 240, 132, 60, 28, hwnd,
			reinterpret_cast<HMENU>(static_cast<INT_PTR>(kCancelButtonID)), nullptr, nullptr
		);
		return 0;
	}
	case WM_COMMAND:
...
bool ShowLoginDialog(LoginDialogResult& result)
{
	...
	HWND dialog = CreateWindowExW(
		WS_EX_DLGMODALFRAME, className, L"Login", WS_CAPTION | WS_SYSMENU | WS_POPUP, x, y, kLoginDialogWidth,
		kLoginDialogHeight, owner, nullptr, instance, &result
	);
...
void LoginSceneControllerComponent::RequestDialog(std::string message, const bool isError)
{
	m_pendingMessage = std::move(message);
	m_pendingMessageIsError = isError;
	m_dialogState = DialogState::ReadyToOpen;
}
...
void LoginSceneControllerComponent::OnUpdate(float deltaTime)
{
	...
	if (m_dialogState != DialogState::ReadyToOpen)
	{
		return;
	}
	...
	{
		const std::wstring message = Utf8ToWide(m_pendingMessage);
		MessageBoxW(
			GetActiveWindow(), message.c_str(), m_pendingMessageIsError ? L"Login Failed" : L"Account Registration",
			MB_OK | (m_pendingMessageIsError ? MB_ICONERROR : MB_ICONINFORMATION)
		);
		m_pendingMessage.clear();
		m_pendingMessageIsError = false;
	}

	m_dialogState = DialogState::DialogOpen;
	...
	if (result.id.empty())
	{
		RequestDialog("Please enter an ID.", true);
		return;
	}

	m_id = result.id;
	m_dialogState = DialogState::WaitingForResponse;
